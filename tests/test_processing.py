...
same_len_lists = (
    ...
    .flatmap(lambda n: st.tuples(st.lists(st.lists(st.text(alphabet=st.characters(whitelist_categories='L'),
                                                           min_size=1, max_size=1),
                                                   min_size=1, max_size=100),
                                          min_size=n, max_size=n),
                                 st.lists(st.text(alphabet=st.characters(whitelist_categories='L'),
                                                  min_size=1),
                                          min_size=n, max_size=n))
             )
)
...
def test_clean_authors(authors):
    ...
    expected = ' '.join([processing.stemmer.stem(n)
                         for n in last_names])
    names = ','.join(['.'.join(fn) + '.' + ln
